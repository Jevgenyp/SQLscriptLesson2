...
FROM staff
ORDER BY lastname
LIMIT 5; -- Первые 5 человек в списке по алфавиту от "А" к "Я"
...
SHOW FULL TABLES; -- Показ таблиц в БД 

-- Создадим архив по месяцам (сколько документов было созданр в каждом месяце) 
...
SELECT
	...
	MONTHNAME(created_at) AS month_name, -- Июнь
    MONTH(created_at) AS month_number -- 06
FROM media
GROUP BY created_at
ORDER BY month_number; 

-- Посчитаем, сколько документов у каждого пользователя 
SELECT
	COUNT(id) AS count_media,
    user_id,
...
SELECT COUNT(DISTINCT post) AS count FROM staff;

SELECT post, AVG(age) AS average_age FROM staff GROUP BY post HAVING average_age < 30;

-- Дополнительные запросы

-- Внутри каждой должности вывести ТОП-2 по заработной плате (2 самых высокооплачиваемых сотрудника внутри каждой должности):
SELECT *
FROM (
    SELECT *,
...
-- Посчитать количество документов у каждого пользователя (документы с расширениями doc, docx, html):

SELECT users.id, users.firstname, users.lastname, COUNT(media.id) AS document_count
FROM users
LEFT JOIN media ON users.id = media.user_id
WHERE media.filename LIKE '%.doc' OR media.filename LIKE '%.docx' OR media.filename LIKE '%.html'
GROUP BY users.id;

-- Посчитать количество лайков для медиафайлов, принадлежащих текущему пользователю:

SELECT media.id, media.filename, COUNT(likes.id) AS like_count
FROM media
LEFT JOIN likes ON media.id = likes.media_id
WHERE media.user_id = [текущий пользователь]
GROUP BY media.id;

...
FROM cats 
INNER JOIN shops ON shops.id = cats.shops_id;

-- Вывести магазин, в котором продается кот "Мурзик". Вы можете сделать это двумя способами: Используя 
-- INNER JOIN:
...
-- Здесь я использую подзапрос, чтобы найти идентификаторы магазинов, в которых продается кот "Murzik" или "Zuza", и затем исключаю их из списка всех магазинов.
SELECT shopname 
FROM shops 
WHERE id NOT IN (
    SELECT shops_id 

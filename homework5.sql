...
-- Создание представления с информацией о пользователях, не старше 20 лет:
CREATE VIEW users_under_20 AS
SELECT firstname, lastname, hometown, gender
FROM users
JOIN profiles ON users.id = profiles.user_id
WHERE DATE_ADD(profiles.birthday, INTERVAL 20 YEAR) >= CURDATE();

-- Подсчет количества отправленных сообщений каждым пользователем и ранжированный список:
SELECT firstname, lastname, message_count, DENSE_RANK() OVER (ORDER BY message_count DESC) AS ranking
FROM (
    SELECT from_user_id, COUNT(*) AS message_count
    FROM messages
    GROUP BY from_user_id
) AS message_counts
JOIN users ON message_counts.from_user_id = users.id;

-- Выбор всех сообщений, сортировка по возрастанию даты отправления и нахождение разницы дат отправления между соседними сообщениями:
